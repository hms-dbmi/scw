...
```{r}
varinfo <- pagoda.varnorm(knn, counts = cd, trim = 3/ncol(cd), max.adj.var = 5, n.cores = 10, plot = TRUE, verbose=TRUE)
```
...
ediff <- scde.expression.difference(knn, cd[vi,], prior=varinfo$prior, groups = test, n.cores = 1, verbose = 1)
```
