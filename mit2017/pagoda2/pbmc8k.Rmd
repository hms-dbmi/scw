...
```{r}
app <- p2.make.pagoda1.app(r,inner.clustering=TRUE,embeddingType='tSNE',clusterType='infomap',min.group.size=50,row.clustering=list(order=rev(1:nrow(r$misc$pathwayOD$xv))))
```
...
Destiny-bsaed embedding:

```{r}
require(destiny)
# variance-scaled matrix
x <- r$counts;
x@x <- x@x*rep(r$misc[['varinfo']][colnames(x),'gsf'],diff(x@p))

dm <- DiffusionMap(as.matrix(x), verbose = TRUE,k=30,n.eigs=50)
